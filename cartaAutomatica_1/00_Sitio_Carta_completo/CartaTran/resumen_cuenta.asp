...
//************************************************//
	...
	sQuery = "EXEC UP_CuentaPeriodoResumen_Existe " + Session("userId")+","+periodoActual
...
//Response.End
	...
	tResumenCuenta.setVar({
		...
		"PAGO_TOTAL"               : "$ " + Number(rs.CuentaPesoSaldoBanel).toFixed(2),
		...
		"TASA_FINANCIACION"        : Number(rs.TasaFinanciacion).toFixed(2),
		"TASA_PUNITORIOS"          : Number(rs.TasaPunitorios).toFixed(2),
		"TASA_TNA"          	   : Number(rs.TasaTNA).toFixed(2),
		"GASTOS_ADM"		   : Number(rs.GastoEmisiónResumen).toFixed(2),
		"SEGURO_VD"		   : Number(rs.CoberturaVidaDesempleo).toFixed(2),
		"MSG_CARDIF"		   : String(rs.LeyendaCardif),
		"MSG_AC"		   : String(rs.LeyendaAC),
		"MSG_RENOVACION"	   : String(rs.LeyendaRenovacion)
...
//************************************************//
	...
	sQuery = "EXEC UP_CuentaPeriodoResumenCabeceraDetalle_VISTA " + Session("userId")+","+periodoActual
//Response.Write(sQuery)
//Response.End
	oDB.query(sQuery)
	iPagoTotal = 0
	while(rs = oDB.each()) {
		...
		tResumenCuenta.setVar({
			"FECHA"       : rs.Fecha != null   ? formatearFecha(rs.Fecha)	 : "&nbsp;",
...
//**************************************//
	...
	sQuery  = "UP_CuentaPeriodoResumenOpcionesDePago_VISTA " + Session("userId")+","+periodoActual
//Response.Write(sQuery)
//Response.End	
	if(oDB.query(sQuery)) {
		...
		while(rs = oDB.each()) {
			...
			if(titulo == "Inicio") {
				...
				tResumenCuenta.setVar({
					...
					"strPA"			 : "$ " + Number(rs.Importe).toFixed(2),
					"strCuota"	         : "y "+rs.CuotasPendi+" cuotas de $ " + Number(rs.MontoCuota).toFixed(2)
<!-- 					"strDIS"		 : Number(rs.MontoCuota).toFixed(2) + "%"   //-->
				})
			...
			if(titulo == "NoInicio") {
				tResumenCuenta.setVar({
					...
					"strPA"			 : "$ " + Number(rs.Importe).toFixed(2),
					"strCuota"	         : "y "+rs.CuotasPendi+((rs.CuotasPendi==1)?" cuota de $ ":" cuotas de $ ") + Number(rs.MontoCuota).toFixed(2)
<!--					"strDIS"		 : Number(rs.MontoCuota).toFixed(2) + "%"   //-->
				})
		...
		tResumenCuenta.setVar({
			"strPA"			 : "$ " + Number(iTotalMes).toFixed(2),
			"strCuota"	         : ""
<!--		"strDIS"		 : ""   //-->
		})
		tResumenCuenta.parse("BLOCK_PlanCuotas", "PlanCuotas", true)
